...
api_key_scheme = APIKeyHeader(
    ...
    description="enter API key in the format: ApiKey <YOUR API KEY>",
)
...
async def get_token_payload(
    token: Annotated[Optional[str], Depends(oauth2_scheme)], token_manager: TokenDep
) -> TokenData:
    if token is None:
        raise HTTPException(
...
async def get_api_payload(
    ...
    return ApiData(user_id=verified_api_key.user_id, role=verified_api_key.user_role)
