...
from engine.chunker.splitters import SentenceSplitter
...
class ParentDocumentRetriever:
    ...
    def _calculate_breakpoint_threshold(
        ...
        if breakpoint_threshold_type == BreakPointThresholdTypeEnum.PERCENTILE:
            return (
                cast(float, np.percentile(distances, breakpoint_threshold_amount)),
                distances,
            )
            ...
            return (q3 + breakpoint_threshold_amount * iqr, distances)
            ...
            return (
                cast(
                    float, np.percentile(distance_gradient, breakpoint_threshold_amount)
                ),
                distance_gradient,
            )
        else:
            raise ValueError(
                f"got unexpected `breakpoint threshold type`: "
                f"{breakpoint_threshold_type}"
            )

    def _calculate_sentence_distances(
        self,
        single_sentence_list: List[str],
        buffer_size: int,
    ) -> Tuple[List[float], List[dict]]:
        _sentences = [
            {"sentence": x, "index": i} for i, x in enumerate(single_sentence_list)
        ]
        sentences = combine_sentences(_sentences, buffer_size=buffer_size)
        embeddings = self.embeddings.embed_documents(
            [x["combined_sentence"] for x in sentences]
        )
        for i, sentence in enumerate(sentences):
            sentence["combined_sentence_embeddings"] = embeddings[i]

        return calculate_cosine_distances(sentences=sentences)
    ...
    def _split_text(
        ...
        single_sentence_list = self.sentence_splitters.split_text(text=text)

        if len(single_sentence_list) == 1:
            return single_sentence_list

        distances, sentences = self._calculate_sentence_distances(
            single_sentence_list=single_sentence_list, buffer_size=buffer_size
        )
        ...
        if len(parent_chunked_docs) != 0:
            for doc in parent_chunked_docs:
                ...
                child_chunked_doc = self._create_documents(
                    child_documents=True, texts=text
                )
